	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case NUMERIC_KEYBOARD_EDIT_PHONE:
			if(resultCode == RESULT_OK){
				...
				if(contactPhones.get(cDataIndexForMaintenance).size() > 2) contactPhones.get(cDataIndexForMaintenance).remove(2);
		...
		case WORD_MAKER_EDIT_EMAIL:
			if(resultCode == RESULT_OK){
				...
				if(contactEmails.get(cDataIndexForMaintenance).size() > 2) contactEmails.get(cDataIndexForMaintenance).remove(2);
